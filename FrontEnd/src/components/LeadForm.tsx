...
export default function LeadForm() {
  ...
  return (
    <form className="mt-8 grid gap-4 md:grid-cols-2" onSubmit={handleSubmit} noValidate>
      ...
      <InputField
        label="Estimated overdue invoices per month"
        value={values.invoiceVolume}
        onChange={(value) => setField('invoiceVolume', value)}
        placeholder="Example: 15 invoices / $25k overdue"
      ...
      <label className="block md:col-span-2">
        ...
        <textarea
          ...
          placeholder="Clients pay late, reminders are inconsistent, and cash flow is unpredictable."
          className="min-h-[140px] w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder:text-slate-400 focus:border-white/30 focus:outline-none"
        />
      ...
      <div className="flex flex-col gap-4 md:col-span-2 md:flex-row md:items-center md:justify-between">
        <div>
          {submitError ? (
            ...
            <p className="text-sm text-slate-400">We’ll use this information to tailor your demo.</p>
          )}
        </div>
        ...
        <button
          ...
          className="inline-flex items-center justify-center rounded-2xl bg-white px-6 py-3 text-sm font-medium text-slate-900 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-60"
        >
